...
# =========================================================================================== #
runs %>% ggplot(aes(x = as.numeric(SDAT), y = HWAM, group = ID)) +
...
# =========================================================================================== #
runs %>% ggplot(aes(x = SDAT, y = HWAM, group = ID, colour = factor(FPU))) +
  geom_line(alpha = .2, size = 1.2) + theme_bw() + xlab('Year') + ylab("Yield (kg/ha)") +
  theme(axis.title.x = element_text(size = 20, face = 'bold'),
        axis.title.y = element_text(size = 20, face = 'bold'),
...
load("./data/management/_maize_crop_mgmt_secano.RDat")
...
col_shp <- raster::getData(name = 'GADM', country = 'COL', level = 2)

ggplot() +
  geom_polygon(data = col_shp, 
               aes(x = long, y = lat, group = group)) +
  coord_fixed() + geom_tile(data = crop_mgmt_climate, aes(x = x, y = y, fill = Rend)) + theme_bw() + scale_fill_distiller(palette = "Spectral") +
...
crop_mgmt_climate$Slope <- magrittr::extract2(crop_mgmt_climate, 'dssat_runs') %>%
  purrr::map(.f = function(x){slope_trend(df = x)}) %>%
  as.numeric()
